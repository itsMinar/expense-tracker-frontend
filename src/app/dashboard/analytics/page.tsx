...
export default function AnalyticsPage() {
  ...
  const expenseData = report?.expenseByCategory
    ? Object.entries(report.expenseByCategory).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
  const incomeData = report?.incomeByCategory
    ? Object.entries(report.incomeByCategory).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
          ...
          <>
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Card className="p-6">
                ...
                <p
                  className={`text-2xl font-bold ${report.summary.netSavings >= 0 ? 'text-emerald-500' : 'text-red-500'}`}
                >
            ...
            <Card className="p-6">
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={[
                      { name: 'Income', amount: report.summary.totalIncome },
                      { name: 'Expense', amount: report.summary.totalExpense },
                    ]}
                  >
                    <CartesianGrid
                      strokeDasharray="3 3"
            ...
            <Card className="p-6">
              ...
              <div className="divide-y">
                {[
                  ...report.expenses.slice(0, 10),
                  ...report.incomes.slice(0, 10),
                ]
                  .sort(
                    (a, b) =>
                  ...
                  .map((t: any) => (
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-medium ${'source' in t ? 'text-emerald-500' : 'text-red-500'}`}
                      >
                        {'source' in t ? '+' : '-'}
                        {fc(t.amount)}
                      </p>
